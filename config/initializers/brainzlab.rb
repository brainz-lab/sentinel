...
BrainzLab.configure do |config|
  ...
  config.service = "sentinel"
  config.environment = Rails.env

  # Ignore internal BrainzLab hosts to prevent infinite recursion
  config.http_ignore_hosts = %w[localhost 127.0.0.1 recall reflex pulse signal flux sentinel]
end
...
Rails.application.config.after_initialize do
  ...
  Rails.logger.info "[Sentinel] SDK initialized"
  Rails.logger.info "[Sentinel] Recall logging: #{BrainzLab.configuration.recall_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Sentinel] Reflex error tracking: #{BrainzLab.configuration.reflex_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Sentinel] Pulse APM: #{BrainzLab.configuration.pulse_enabled ? 'enabled' : 'disabled'}"
end
